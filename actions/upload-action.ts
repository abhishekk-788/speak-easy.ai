...
export async function transcribeUploadedFile(
  ...
  console.log("Response Data", userId, fileName, fileUrl);
  ...
  if (fileExtension && !supportedFormats.includes(fileExtension)) {
    ...
    return {
      success: false,
      message: `Unsupported file format: ${fileExtension}. Supported formats are: ${supportedFormats.join(
        ", "
      )}`,
      data: null,
    };
  ...
  try {
    ...
    const plainTranscriptions = JSON.parse(JSON.stringify(transcriptions));

    console.log({ transcriptions: plainTranscriptions });
    return {
      success: true,
      message: "File uploaded successfully!",
      data: { transcriptions: plainTranscriptions, userId },
    };
  } catch (error) {
